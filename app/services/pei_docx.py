...
def gerar_resumo_trimestral_local(df_hist_periodo, df_alvos_periodo, df_beh_periodo, objetivo_grafico, inicio, fim):
    ...
    if not df_obj.empty and "independent_rate" in df_obj.columns:
        ...
        partes.append(f"No periodo, {alvo_txt} reuniu {registros} registros de ensino.")

        resumo_programas = []
        grupos = df_obj.groupby("programa") if "programa" in df_obj.columns else [(alvo_txt, df_obj)]
        for programa, grupo in grupos:
            sessoes = prepare_program_sessions(grupo)
            if sessoes.empty:
                continue
            metricas = calculate_clinical_metrics(sessoes)
            if metricas.latest is None:
                continue
            media = metricas.weighted_mean if metricas.weighted_mean is not None else metricas.simple_mean
            resumo_programas.append(
                f"{limpar_nome_objetivo(programa)}: nível médio {media:.1f}%, "
                f"último registro {metricas.latest:.1f}%, tendência {classify_trend(metricas.slope).casefold()} "
                f"({metricas.slope if metricas.slope is not None else 0:.2f} ponto(s) por observação), "
                f"variabilidade {metricas.variability.casefold()}, {metricas.sessions} registros"
            )
        if resumo_programas:
            partes.append("Desempenho por objetivo (nível, tendência e variabilidade): " + "; ".join(resumo_programas[:6]) + ".")

        if "objective" in df_obj.columns:
            objetivos = [str(v).strip() for v in df_obj["objective"].dropna().unique() if str(v).strip()]
            ajustes = []
            for objetivo in objetivos[:5]:
                avaliacao = avaliar_objetivo_observavel(objetivo)
                if avaliacao["faltantes"]:
                    ajustes.append(f"{limpar_texto_pei(objetivo)} — {avaliacao['recomendacao']}")
            if ajustes:
                partes.append("Qualidade operacional dos objetivos: " + "; ".join(ajustes) + ".")
    else:
        partes.append("Não há registros de objetivos no ciclo selecionado; ausência de registro não equivale a ausência de aplicação.")

    if not df_alvos_periodo.empty and "target_name" in df_alvos_periodo.columns:
        alvos = summarize_targets(df_alvos_periodo)
        if not alvos.empty:
            destaques = []
            for _, alvo in alvos.head(6).iterrows():
                media = alvo.get("weighted_mean")
                tipo_media = "ponderada por tentativas"
                if pd.isna(media):
                    media = alvo.get("simple_mean")
                    tipo_media = "simples, sem denominador de tentativas"
                if pd.isna(media):
                    continue
                oportunidades = alvo.get("opportunities")
                oportunidade_txt = f", {int(oportunidades)} oportunidades" if pd.notna(oportunidades) else ""
                destaques.append(
                    f"{limpar_texto_pei(alvo['target_name'])}: {media:.1f}% ({tipo_media}), "
                        f"{int(alvo['sessions'])} datas{oportunidade_txt}"
                )
            partes.append("Alvos com melhor desempenho observado: " + "; ".join(destaques) + ".")

    if not df_beh_periodo.empty and "comportamento" in df_beh_periodo.columns:
        resumo_beh = summarize_interfering_behaviors(df_beh_periodo)
        itens_beh = []
        for _, item in resumo_beh.head(6).iterrows():
            valor = item["mean_rate"] if item["measure"] == "taxa" else item["total_count"]
            itens_beh.append(
                f"{limpar_texto_pei(item['behavior'])}: {item['measure']}={valor}, "
                f"tendência {str(item['trend']).casefold()}, variabilidade {str(item['variability']).casefold()}, "
                f"{int(item['observed_days'])} dias observados"
            )
        if itens_beh:
            partes.append("Comportamentos interferentes (descrição sem inferência de função): " + "; ".join(itens_beh) + ".")
        qualidade = behavior_quality_issues(df_beh_periodo)
        if qualidade["missing_measure"] or qualidade["invalid_dates"] or qualidade["potential_duplicates"]:
            partes.append(
                "Limites dos dados de comportamento: "
                f"{qualidade['missing_measure']} registros sem medida, {qualidade['invalid_dates']} datas inválidas e "
                f"{qualidade['potential_duplicates']} possíveis duplicidades."
            )
        if not ("rate" in df_beh_periodo and pd.to_numeric(df_beh_periodo["rate"], errors="coerce").notna().any()):
            partes.append("Contagens entre sessões só são comparáveis quando a duração ou a oportunidade de observação é equivalente.")
    else:
        partes.append("Não há registros de comportamentos interferentes no ciclo selecionado; ausência de registro não equivale a ausência do comportamento.")

    evolucoes = coletar_evolucoes_periodo(df_hist_periodo, df_alvos_periodo, df_beh_periodo, limite=8)
    if evolucoes:
...
def gerar_texto_trimestral_pei(
    ...
    pergunta = (
        ...
        "Separe claramente observacao dos dados, inferencia e recomendacao. Descreva nivel, tendencia e variabilidade; "
        "nao conclua dominio a partir de um unico resultado alto e diferencie criterio de desempenho de manutencao e generalizacao. "
        "Ausencia de registro nao significa ausencia do comportamento. Nao atribua funcao a comportamento interferente sem dados "
        "de antecedentes, resposta e consequencias; quando faltarem, declare a limitacao e proponha avaliacao funcional. "
        "Toda sugestao de objetivo deve indicar resposta observavel, contexto, medida e criterio quantitativo individualizado. "
...
def gerar_resumo_comportamentos_problema_local(df_beh_periodo, inicio, fim):
    ...
    if df_beh_periodo.empty or "comportamento" not in df_beh_periodo.columns:
        return (
            f"No periodo de {periodo}, não há registros de comportamentos interferentes; "
            "ausência de registro não equivale a ausência do comportamento."
        )

    partes = [f"Resumo dos comportamentos interferentes no periodo de {periodo}."]
    resumo = summarize_interfering_behaviors(df_beh_periodo)
    for _, item in resumo.head(8).iterrows():
        valor = item["mean_rate"] if item["measure"] == "taxa" else item["total_count"]
        partes.append(
            f"{item['behavior']}: {item['measure']}={valor}, último valor={item['latest_value']}, "
            f"tendência {str(item['trend']).casefold()}, variabilidade {str(item['variability']).casefold()} "
            f"em {int(item['observed_days'])} dias observados."
        )
    partes.append(
        "A função do comportamento não pode ser determinada por estes totais; hipóteses funcionais exigem "
        "dados de antecedentes, resposta e consequências e avaliação funcional supervisionada."
    )
...
def gerar_resumo_comportamentos_problema_pei(
    ...
    pergunta = (
        ...
        "Separe fatos registrados de hipoteses. Descreva nivel, tendencia e variabilidade. Ausencia de registro nao e zero. "
        "Nao atribua funcao ao comportamento sem dados de antecedentes, resposta e consequencias; explicite essa lacuna e "
        "recomende avaliacao funcional supervisionada. Nao prescreva intervencao automaticamente a partir de correlacoes. "
...
def gerar_doc_completo(
    ...
    for _, row in df_prog_unicos.iterrows():
        ...
        if not df_alvos_periodo.empty and "programa" in df_alvos_periodo.columns:
            ...
            if not alvos_prog.empty:
                ...
                mask = pd.Series(True, index=alvos_prog.index)

                if not alvos_prog[mask].empty:
                    sessoes_requeridas = sessoes_dominio_programa(prog_nome, df_lib)
                    alvos_meta = []
                    for alvo, grupo_alvo in alvos_prog[mask].groupby("target_name"):
                        criterio_alvo = evaluate_performance_criterion(
                            grupo_alvo, limiar, sessoes_requeridas
                        )
                        if criterio_alvo["performance_criterion_achieved"]:
                            alvos_meta.append((alvo, criterio_alvo))
                    total_alvos_atingidos += len(alvos_meta)
                    if alvos_meta:
                        for alvo, criterio_alvo in sorted(alvos_meta, key=lambda item: item[0]):
                            doc.add_paragraph(
                                f"  - {alvo}: critério de desempenho atingido "
                                f"({criterio_alvo['streak']}/{criterio_alvo['required']} datas); "
                                "manutenção e generalização devem ser verificadas separadamente."
                            )
                    else:
                        doc.add_paragraph("  - Nenhum novo alvo atingiu o critério de desempenho configurado.")
                else:
                    doc.add_paragraph("  - Nenhum novo alvo atingiu o critério de desempenho configurado.")
            else:
    ...
    if total_alvos_trabalhados > 0:
        ...
        for bar in bars:
            yval = bar.get_height()
            escala = max(total_alvos_trabalhados, total_alvos_atingidos, 1)
            plt.text(bar.get_x() + bar.get_width() / 2, yval + escala * 0.05, int(yval), ha="center", va="bottom", fontweight="bold")
        plt.ylim(0, max(total_alvos_trabalhados, total_alvos_atingidos, 1) * 1.25)
        plt.tight_layout()
        buf_alvos = io.BytesIO()
        plt.savefig(buf_alvos, format="png")
        plt.close()
        buf_alvos.seek(0)
        doc.add_picture(buf_alvos, width=Inches(5.7))
