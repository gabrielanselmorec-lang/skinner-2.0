...
DEFAULT_KNOWLEDGE_PDF_DIR = Path("dados_clinica") / "base_conhecimento_pdf"
KNOWLEDGE_PDF_CONFIG_NAME = "base_conhecimento_pdf_dirs.txt"
...
TOKEN_RE = re.compile(r"[^\W_]{3,}", flags=re.UNICODE)
...
class KnowledgeSnapshot:
    ...
    source_issues: tuple[tuple[str, str], ...]
    markdown_files: int
    pdf_files: int
    pdf_pages: int
...
def knowledge_pdf_dirs() -> tuple[Path, ...]:
    configured = os.getenv("SKINNER_KNOWLEDGE_PDF_DIRS", "").strip()
    if not configured:
        configured = os.getenv("SKINNER_KNOWLEDGE_PDF_DIR", "").strip()

    candidates: list[Path] = []
    if configured:
        candidates.extend(Path(part.strip()) for part in configured.split(os.pathsep) if part.strip())
    config_path = knowledge_dir().parent / KNOWLEDGE_PDF_CONFIG_NAME
    if config_path.is_file():
        for line in config_path.read_text(encoding="utf-8-sig").splitlines():
            value = line.strip()
            if value and not value.startswith("#"):
                candidates.append(Path(value))
    default_pdf_dir = Path.cwd() / DEFAULT_KNOWLEDGE_PDF_DIR
    if default_pdf_dir.is_dir() or not candidates:
        candidates.append(DEFAULT_KNOWLEDGE_PDF_DIR)
...
def knowledge_cache_dir() -> Path:
    configured = os.getenv("SKINNER_KNOWLEDGE_CACHE_DIR", "").strip()
    base = Path(configured) if configured else knowledge_dir().parent / ".knowledge_cache"
    if not base.is_absolute():
        base = Path.cwd() / base
    base = base.resolve()
    base.mkdir(parents=True, exist_ok=True)
...
def _canonical_source_key(source: str) -> str:
    ...
    return re.sub(r"[^a-z0-9]+", "", _normalize_search_text(stem))


def _tokens(text: str) -> list[str]:
    safe = sanitize_text(text, max_length=120000, allow_newlines=True)
    safe = _normalize_search_text(safe)
...
def _read_markdown(path: Path) -> str:
    if path.stat().st_size > MAX_MARKDOWN_BYTES:
        raise ValueError(
            f"arquivo Markdown excede o limite de {MAX_MARKDOWN_BYTES // 1_000_000} MB"
        )
...
def _split_markdown(text: str) -> list[str]:
    ...
    for section in sections:
        section = sanitize_for_ai_context(section, max_length=max(len(section), 1))
...
def _knowledge_entries() -> tuple[tuple[str, str, str, int, int], ...]:
    entries: list[tuple[str, str, str, int, int]] = []
    markdown_root = ensure_knowledge_dir()
    for path in sorted(markdown_root.rglob("*.md")):
        safe_path = ensure_child_path(markdown_root, path)
        stat = safe_path.stat()
        entries.append(("markdown", str(markdown_root), str(safe_path), stat.st_size, stat.st_mtime_ns))

    seen_pdfs: set[Path] = set()
    for pdf_root in knowledge_pdf_dirs():
        if not pdf_root.is_dir():
            continue
        for path in sorted(pdf_root.rglob("*")):
            if not path.is_file() or path.suffix.casefold() != ".pdf":
                continue
            safe_path = ensure_child_path(pdf_root, path)
            if safe_path in seen_pdfs:
                continue
            seen_pdfs.add(safe_path)
            stat = safe_path.stat()
            entries.append(("pdf", str(pdf_root), str(safe_path), stat.st_size, stat.st_mtime_ns))
    return tuple(sorted(entries, key=lambda item: (item[0], item[2].casefold())))


def _pdf_cache_path(path: Path, cache_root: Path) -> Path:
    key = hashlib.sha256(str(path).casefold().encode("utf-8")).hexdigest()[:24]
    return ensure_child_path(cache_root, cache_root / f"pdf_{key}.json")


def _load_pdf_pages(path: Path, cache_root: Path) -> dict[str, Any]:
    stat = path.stat()
    if stat.st_size > MAX_PDF_BYTES:
        raise ValueError(f"arquivo PDF excede o limite de {MAX_PDF_BYTES // 1_000_000} MB")

    cache_path = _pdf_cache_path(path, cache_root)
    if cache_path.is_file():
        try:
            cached = json.loads(cache_path.read_text(encoding="utf-8"))
            if (
                cached.get("schema") == PDF_CACHE_SCHEMA
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError("pypdf nao esta instalado; nao foi possivel indexar os PDFs") from exc
    ...
    for page_number, page in enumerate(reader.pages, start=1):
        try:
            ...
            text = sanitize_for_ai_context(raw_text, max_length=max(len(raw_text), 1))
        except Exception as exc:
            text = ""
            errors.append({"pagina": page_number, "erro": type(exc).__name__})
        pages.append({"pagina": page_number, "texto": text})
    ...
    payload: dict[str, Any] = {
        ...
        "titulo": sanitize_text(metadata.get("/Title") or path.stem, max_length=240),
        "autor": sanitize_text(metadata.get("/Author") or "", max_length=240),
        "paginas": pages,
        "erros": errors,
    }
...
@lru_cache(maxsize=1)
def _build_snapshot(
    cache_root_value: str,
    entries: tuple[tuple[str, str, str, int, int], ...],
) -> KnowledgeSnapshot:
    cache_root = Path(cache_root_value)
    chunks: list[KnowledgeChunk] = []
    issues: list[tuple[str, str]] = []
    markdown_files = sum(kind == "markdown" for kind, *_ in entries)
    pdf_files = sum(kind == "pdf" for kind, *_ in entries)
    pdf_pages = 0
    pdf_pages_with_text = 0

    for kind, root_value, path_value, _size, _mtime_ns in entries:
        ...
        try:
            if kind == "markdown":
                ...
                if not content.strip():
                    issues.append((relative, "arquivo vazio ou sem texto legivel"))
                    continue
            ...
            payload = _load_pdf_pages(path, cache_root)
            source = f"pdf/{relative}"
            document_title = sanitize_text(payload.get("titulo") or path.stem, max_length=200)
            chunk_index = 0
            pages = payload.get("paginas") or []
            pdf_pages += len(pages)
            empty_pages = 0
            for page_data in pages:
                page_number = int(page_data.get("pagina") or 0)
                page_text = str(page_data.get("texto") or "").strip()
                if not page_text:
                    empty_pages += 1
                    continue
                pdf_pages_with_text += 1
                for chunk_text in _split_markdown(page_text):
                    chunk_index += 1
                    chunks.append(
                        KnowledgeChunk(
                            source=source,
                            title=f"{document_title} - pagina {page_number}",
                            text=sanitize_text(chunk_text, max_length=4000, allow_newlines=True),
            ...
            if empty_pages:
                issues.append((source, f"{empty_pages} pagina(s) sem texto extraivel"))
            extraction_errors = payload.get("erros") or []
            if extraction_errors:
                issues.append((source, f"falha de extracao em {len(extraction_errors)} pagina(s)"))
        except Exception as exc:
            issues.append((relative, sanitize_text(exc, max_length=240)))

    chunk_token_counts_list: list[dict[str, int]] = []
    document_frequency: Counter[str] = Counter()
    for chunk in chunks:
        token_counts = dict(Counter(_tokens(f"{chunk.title}\n{chunk.text}")))
        chunk_token_counts_list.append(token_counts)
        document_frequency.update(token_counts.keys())
    chunk_token_counts = tuple(chunk_token_counts_list)
    return KnowledgeSnapshot(
        ...
        pdf_files=pdf_files,
        pdf_pages=pdf_pages,
        pdf_pages_with_text=pdf_pages_with_text,
    )


def _snapshot() -> KnowledgeSnapshot:
    return _build_snapshot(str(knowledge_cache_dir()), _knowledge_entries())


def clear_knowledge_memory_cache() -> None:
    _build_snapshot.cache_clear()


def load_chunks() -> list[KnowledgeChunk]:
    return list(_snapshot().chunks)


def status() -> dict[str, Any]:
    base = ensure_knowledge_dir()
    snapshot = _snapshot()
    pdf_dirs = knowledge_pdf_dirs()
    return {
        "diretorio": str(base),
        "diretorios_pdf": [str(path) for path in pdf_dirs],
        "diretorios_pdf_ausentes": [str(path) for path in pdf_dirs if not path.is_dir()],
        "diretorio_cache": str(knowledge_cache_dir()),
        "arquivos_md": snapshot.markdown_files,
        "arquivos_pdf": snapshot.pdf_files,
        "paginas_pdf": snapshot.pdf_pages,
        "paginas_pdf_com_texto": snapshot.pdf_pages_with_text,
        "trechos_indexados": len(snapshot.chunks),
        "fontes_indexadas": len({chunk.source for chunk in snapshot.chunks}),
        "alertas_fontes": [
            {"fonte": source, "detalhe": detail}
            for source, detail in snapshot.source_issues
        ],
...
def search(query: str, *, limit: int = 6) -> list[dict[str, Any]]:
    ...
    total_docs = max(len(snapshot.chunks), 1)
    for chunk, token_counts in zip(
        snapshot.chunks,
        snapshot.chunk_token_counts,
    ):
        if not token_counts:
            continue
        score = 0.0
        for token, query_count in query_counts.items():
            ...
            if not tf:
                continue
            idf = math.log((1 + total_docs) / (1 + snapshot.document_frequency.get(token, 0))) + 1
    ...
    for item_index, item in enumerate(scored):
        source_key = _canonical_source_key(item[1].source)
        if per_source[source_key] >= 2:
            continue
        selected.append(item)
        selected_ids.add(item_index)
    ...
    for score, chunk in selected:
        result = {
            ...
            "tipo_fonte": chunk.source_kind,
            "score": round(score, 4),
        }
        if chunk.page_number is not None:
            result["pagina"] = chunk.page_number
...
def _summarize_patient_context(patient_context: dict[str, Any]) -> str:
    ...
    if targets:
        ...
        target_metrics = summarize_targets(pd.DataFrame(targets))
        for _, item in target_metrics.head(20).iterrows():
            weighted = item.get("weighted_mean")
            simple = item.get("simple_mean")
            chosen = weighted if pd.notna(weighted) else simple
            mean_kind = "ponderada_por_tentativas" if pd.notna(weighted) else "simples_sem_denominador"
            lines.append(
                "- resumo {target}: independencia={value}, tipo_media={kind}, datas={sessions}, "
                "oportunidades={opportunities}, ultimo={latest}, inclinacao={slope}".format(
                    target=sanitize_text(item.get("target_name", ""), max_length=160),
                    value=chosen,
                    kind=mean_kind,
        ...
        for item in targets:
            ...
            for field, bucket in [
                ...
                if raw_value in (None, ""):
                    continue
                try:
                    summary[bucket].append(float(raw_value))
        ...
        for item in targets[:30]:
            ...
            lines.append(linha)
    if behaviors:
        lines.append("Comportamentos interferentes (descrição quantitativa; não implica função):")
        behavior_frame = pd.DataFrame(behaviors)
        behavior_metrics = summarize_interfering_behaviors(behavior_frame)
        for _, item in behavior_metrics.head(12).iterrows():
            value = item["mean_rate"] if item["measure"] == "taxa" else item["total_count"]
            lines.append(
                "- {behavior}: medida={measure}, nivel={value}, ultimo={latest}, tendencia={trend}, "
                "variabilidade={variability}, dias_observados={days}, registros={records}".format(
                    behavior=sanitize_text(item["behavior"], max_length=120),
                    measure=item["measure"],
                    value=value,
                    latest=item["latest_value"],
                    trend=item["trend"],
                    variability=item["variability"],
                    days=item["observed_days"],
                    records=item["records"],
                )
            )
        behavior_issues = behavior_quality_issues(behavior_frame)
        lines.append(
            "Limites dos dados de interferentes: registros_sem_medida={missing}, datas_invalidas={dates}, "
            "possiveis_duplicidades={duplicates}. Ausencia de registro nao equivale a zero; funcao nao foi medida.".format(
                missing=behavior_issues["missing_measure"],
                dates=behavior_issues["invalid_dates"],
                duplicates=behavior_issues["potential_duplicates"],
            )
        )
        ...
        for item in behaviors:
            ...
            try:
                if item.get("rate") not in (None, ""):
                    summary["rates"].append(float(item.get("rate")))
            except (TypeError, ValueError):
                pass
            if item.get("date"):
                summary["dates"].append(str(item.get("date")))
...
def _format_reference_bibliography(references: list[dict[str, Any]]) -> str:
    ...
    for index, ref in enumerate(references, start=1):
        ...
        page_number = ref.get("pagina")
        if page_number is not None:
            details.append(f"pagina: {page_number}")
        if chunk_index != "":
...
6. OBSERVAÇÃO ≠ INFERÊNCIA: Separe explicitamente o que foi medido, o que é hipótese e o que é recomendação. Não atribua causalidade a correlações.
7. LEITURA DOS DADOS: Considere conjuntamente nível, tendência e variabilidade. Não trate média isolada como tendência. Contagens entre sessões só são comparáveis quando o tempo ou a oportunidade de observação é equivalente.
8. REGISTRO AUSENTE ≠ ZERO: Nunca diga que um comportamento não ocorreu apenas porque não há registro. Diferencie zero observado, dado ausente e período sem coleta.
9. CRITÉRIO ≠ DOMÍNIO: Um resultado alto ou o alcance de um critério percentual não comprova sozinho domínio. Informe se manutenção e generalização foram ou não registradas.
10. FUNÇÃO DO COMPORTAMENTO: Não determine função a partir de totais, taxas ou relatos isolados. Trate hipóteses funcionais como provisórias e solicite dados de antecedentes, resposta e consequências e avaliação funcional supervisionada.
11. OBJETIVOS OBSERVÁVEIS: Ao sugerir objetivo, inclua comportamento observável, condição/contexto, dimensão de medida e critério quantitativo individualizado; inclua manutenção e generalização quando pertinentes.
12. DECISÃO HUMANA: Não selecione automaticamente tratamento. Apresente opções condicionais à função e aos dados, para revisão do supervisor clínico.
...
[Faça um resumo objetivo, descritivo e cauteloso. Separe nível, tendência e variabilidade; informe cobertura e limites dos dados, incluindo manutenção/generalização não medidas.]

### 2. Hipóteses Analítico-Comportamentais
[Levante hipóteses clínicas provisórias para investigação. Analise possíveis correlações entre desempenho, nível de ajuda, fases e comportamentos interferentes sem afirmar causalidade ou função não medida. Diga quais dados adicionais testariam cada hipótese. Cite as fontes, ex: (Fonte 1).]

### 3. Sugestões de Próximos Passos (Observáveis)
[Forneça sugestões práticas, mensuráveis e condicionais aos dados para a equipe clínica. Para objetivos, especifique resposta, contexto, medida e critério; inclua sondagens de manutenção e generalização. Cite as fontes quando aplicável.]
