...
def sessoes_dominio_programa(programa, df_lib, padrao=3):
    if df_lib.empty or "name" not in df_lib.columns or not programa:
        return max(1, int(padrao))
    registros = df_lib[df_lib["name"] == programa]
    if registros.empty:
        return max(1, int(padrao))
    valor = registros.iloc[0].get("mastery_days", padrao)
    try:
        return max(1, int(valor)) if pd.notna(valor) else max(1, int(padrao))
    except (TypeError, ValueError):
        return max(1, int(padrao))


def avaliar_objetivo_observavel(texto):
    """Verifica componentes mínimos sem substituir julgamento clínico individualizado."""
    objetivo = limpar_texto_pei(texto or "")
    texto_cf = objetivo.casefold()
    componentes = {
        "comportamento_observavel": bool(re.search(
            r"\b(nomear|identificar|selecionar|solicitar|responder|seguir|realizar|imitar|"
            r"iniciar|manter|reduzir|aumentar|esperar|tolerar|comunicar|entregar|apontar|"
            r"ler|escrever|brincar|vestir|lavar|escovar)\w*\b", texto_cf
        )),
        "condicao_contexto": bool(re.search(
            r"\b(diante de|quando|durante|após|apos|em atividades|em contexto|com)\b", texto_cf
        )),
        "medida": bool(re.search(
            r"(%|percentual|taxa|frequência|frequencia|duração|duracao|latência|latencia|tentativas?)", texto_cf
        )),
        "criterio_quantitativo": bool(re.search(
            r"\b\d+(?:[.,]\d+)?\s*(?:%|vez(?:es)?|minutos?|segundos?|tentativas?|sessões|sessoes|dias?)\b", texto_cf
        )),
        "generalizacao_manutencao": bool(re.search(
            r"generaliza|manuten[cç][aã]o|diferentes pessoas|diferentes ambientes|diferentes materiais", texto_cf
        )),
    }
    rotulos = {
        "comportamento_observavel": "comportamento observável",
        "condicao_contexto": "condição ou contexto",
        "medida": "dimensão de medida",
        "criterio_quantitativo": "critério quantitativo",
        "generalizacao_manutencao": "generalização/manutenção",
    }
    faltantes = [rotulos[chave] for chave, presente in componentes.items() if not presente]
    return {
        **componentes,
        "pontuacao": sum(componentes.values()),
        "total": len(componentes),
        "faltantes": faltantes,
        "recomendacao": "Objetivo operacionalmente completo." if not faltantes else "Acrescentar: " + ", ".join(faltantes) + ".",
    }
...
def resumo_alvos_programa(df_alvos, programa, df_lib, df_alvos_periodo=None):
    ...
    sessoes_requeridas = sessoes_dominio_programa(programa, df_lib)
    rows = []
    for alvo, grupo_periodo in df_prog_periodo.groupby("target_name"):
        ...
        inicial = serie_lb.mean() if not serie_lb.empty else pd.NA
        atual = serie_periodo.iloc[-1] if not serie_periodo.empty else pd.NA
        media_ponderada = calculate_weighted_independence(grupo_periodo)
        media_ind = media_ponderada if media_ponderada is not None else (
            serie_periodo.mean() if not serie_periodo.empty else pd.NA
        )
        criterio = evaluate_performance_criterion(grupo_periodo, limiar, sessoes_requeridas)
        linha_base_txt = (
        ...
        periodo_txt = (
            ...
            f"({criterio['distinct_dates']} datas; sequência {criterio['streak']}/{criterio['required']})"
            if pd.notna(media_ind) else "período observado sem registro percentual"
        )
        status = (
            "Atingiu"
            if criterio["performance_criterion_achieved"]
            else (
                "Em avaliação"
                if criterio["current_rate"] is not None and criterio["current_rate"] >= limiar
                else status_por_independencia(inicial, atual, limiar)
            )
        )
        rows.append({
            ...
            "Periodo": periodo_txt,
            "Resumo": f"{linha_base_txt}; {periodo_txt}. {criterio['status']}",
            "Status": status,
        })
...
def resumo_objetivos_por_area(areas):
    ...
    for area_num, itens in areas.items():
        for item_index, item in enumerate(itens):
            avaliacao = avaliar_objetivo_observavel(item.get("texto", ""))
            rows.append({
                "Objetivo": f"Objetivo {area_num}.{item_index + 1}",
                "Descrição": limpar_texto_pei(item.get("texto", "")),
                "Completude": f"{avaliacao['pontuacao']}/{avaliacao['total']}",
                "Ajustes necessários": avaliacao["recomendacao"],
            })
        if not itens:
            rows.append({
                "Objetivo": "-",
                ...
                "Completude": "0/5",
                "Ajustes necessários": "Cadastrar objetivo individualizado.",
            })
    return pd.DataFrame(rows, columns=["Objetivo", "Descrição", "Completude", "Ajustes necessários"])
...
def status_geral_alvos(alvos):
    if alvos.empty:
        return "Sem registro"
    status = set(alvos["Status"].dropna().astype(str))
    if "Agravou" in status:
        return "Agravou"
    if status and status == {"Atingiu"}:
        return "Atingiu"
    if status.intersection({"Avançou", "Atingiu"}):
        return "Avançou"
    if "Em avaliação" in status:
        return "Em avaliação"
    if not status or status == {"Sem registro"}:
        return "Sem registro"
    return "Estabilizou"
...
def distribuir_objetivos_por_area(df_prog, df_beh):
    ...
    if not df_beh.empty and "comportamento" in df_beh.columns:
        ...
        if comportamentos:
            taxa_disponivel = (
                "rate" in df_beh
                and pd.to_numeric(df_beh["rate"], errors="coerce").notna().any()
            )
            medida = "taxa por unidade de tempo" if taxa_disponivel else "frequência por sessão"
            texto = (
                f"Reduzir {medida} dos comportamentos interferentes monitorados "
                f"({', '.join(comportamentos[:8])}) em relação à linha de base, com definição operacional; "
                "ensinar e medir uma resposta alternativa funcionalmente equivalente após avaliação funcional. "
                "Individualizar critério quantitativo, manutenção e generalização."
            )
