...
from app.services.dataframe_utils import filter_clinical_date_range, records_with_nulls
...
def _filter_date_range(df: pd.DataFrame, data_inicio: Optional[str], data_fim: Optional[str]) -> pd.DataFrame:
    # Em chamadas Python diretas, o FastAPI mantém os objetos ``Query`` como
    # defaults. Somente strings efetivamente fornecidas devem virar limites.
    start = data_inicio if isinstance(data_inicio, str) else None
    end = data_fim if isinstance(data_fim, str) else None
    return filter_clinical_date_range(df, start, end)
...
def listar_pacientes():
    try:
        ...
        safe_error = sanitize_text(exc, max_length=200)
        logger.error("Erro ao listar pacientes: %s", safe_error)
        raise HTTPException(status_code=500, detail="Não foi possível listar os pacientes.") from exc
...
def obter_dados_paciente(
    ...
    try:
        ...
        q_beh = text("""
            SELECT b.name as comportamento, ir.date, ir.therapist,
                   ir.count, ir.rate, ir.evolution
        ...
        q_targets = text("""
            SELECT p.name as programa, p.objective, tr.target_name, tr.attempts,
                   ...
                   tr.date, tr.evolution,
                   (
                       SELECT pr_phase.phase
                       FROM program_records pr_phase
                       WHERE pr_phase.program_id = tr.program_id
                         AND pr_phase.date = tr.date
                       ORDER BY pr_phase.id DESC
                       LIMIT 1
                   ) AS phase
        ...
        df_p = pd.read_sql(q_prog, engine, params=params)
        df_b = pd.read_sql(q_beh, engine, params=params)
        include_targets = incluir_alvos if isinstance(incluir_alvos, bool) else True
        df_t = pd.read_sql(q_targets, engine, params=params) if include_targets else pd.DataFrame()
        ...
        return {
            "programas": records_with_nulls(df_p),
            "interferentes": records_with_nulls(df_b),
            "alvos": records_with_nulls(df_t),
        }
    except Exception as exc:
        safe_error = sanitize_text(exc, max_length=200)
        logger.error("Erro ao buscar dados do paciente: %s", safe_error)
        raise HTTPException(status_code=500, detail="Não foi possível carregar os dados clínicos.") from exc
...
def obter_alvos(
    ...
    try:
        ...
        query = text("""
            SELECT tr.id, tr.program_id, tr.target_name, tr.attempts,
                   tr.independent_rate, tr.prompt_rate, tr.success_rate,
                   tr.date, tr.prompt_type, tr.evolution,
                   (
                       SELECT pr_phase.phase
                       FROM program_records pr_phase
                       WHERE pr_phase.program_id = tr.program_id
        ...
        return records_with_nulls(df)
    except Exception as exc:
        safe_error = sanitize_text(exc, max_length=200)
        logger.error("Erro ao buscar alvos: %s", safe_error)
        raise HTTPException(status_code=500, detail="Não foi possível carregar os alvos.") from exc
...
def get_biblioteca():
    ...
    try:
        ...
        safe_error = sanitize_text(exc, max_length=200)
        logger.error("Erro ao carregar biblioteca: %s", safe_error)
        raise HTTPException(status_code=500, detail="Não foi possível carregar a biblioteca.") from exc
    finally:
        session.close()
