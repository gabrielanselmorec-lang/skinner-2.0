...
class TestResumoAlvosPrograma:
    ...
    def test_primeiro_dado_de_intervencao_nao_e_rotulado_como_linha_de_base(self):
        df_alvos = _alvos("Prog A", ["Alvo 1"], ["2026-07-10"], [65], ["Intervenção"])
        resultado = resumo_alvos_programa(df_alvos, "Prog A", pd.DataFrame(), df_alvos)
        assert resultado.iloc[0]["LinhaBase"] == "linha de base sem registro percentual"

    def test_resultado_alto_unico_fica_em_avaliacao(self):
        df_alvos = _alvos("Prog A", ["Alvo 1"], ["2026-07-10"], [95])
        df_lib = pd.DataFrame({"name": ["Prog A"], "mastery_days": [3]})
        resultado = resumo_alvos_programa(df_alvos, "Prog A", df_lib, df_alvos)
        assert resultado.iloc[0]["Status"] == "Em avaliação"
        assert "sequência 1/3" in resultado.iloc[0]["Periodo"]

    def test_atingiu_requer_sequencia_em_datas_distintas(self):
        df_alvos = _alvos(
            "Prog A",
            ["Alvo 1", "Alvo 1", "Alvo 1"],
        ...
        assert "manutenção e generalização pendentes" in resultado.iloc[0]["Resumo"]
...
class TestStatusGeralAlvos:
    def test_resultado_misto_nao_esconde_alvos_nao_atingidos(self):
        df = pd.DataFrame({"Status": ["Estabilizou", "Avançou", "Atingiu"]})
        assert status_geral_alvos(df) == "Avançou"

    def test_agravamento_tem_prioridade_clinica(self):
        df = pd.DataFrame({"Status": ["Atingiu", "Agravou"]})
        assert status_geral_alvos(df) == "Agravou"
...
def test_avaliacao_objetivo_identifica_componentes_ausentes():
    incompleto = avaliar_objetivo_observavel("Melhorar comunicação")
    completo = avaliar_objetivo_observavel(
        "Solicitar ajuda quando houver demanda em 80% das tentativas por 3 sessões, "
        "com manutenção e generalização em diferentes ambientes."
    )
    assert incompleto["pontuacao"] < completo["pontuacao"]
    assert completo["pontuacao"] == completo["total"]


def test_objetivo_interferente_nao_inventa_funcao():
    areas = distribuir_objetivos_por_area(
        pd.DataFrame(),
        pd.DataFrame(
            {"comportamento": ["Fuga de demanda"], "count": [2], "rate": [0.5]}
        ),
    )
    texto = areas[4][0]["texto"]
    assert "após avaliação funcional" in texto
    assert "funcionalmente equivalente" in texto
    assert "atenção" not in texto.casefold()
